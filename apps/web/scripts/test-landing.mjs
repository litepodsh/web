...
assert.deepEqual(locales.en.heroTerms, ["apps.", "databases.", "services.", "infrastructure."]);
assert.equal(locales.en.heroTerms.length, locales.es.heroTerms.length);
...
assert.match(hero, /data-i18n="statsTotal"/);
assert.match(locales.en.statsTotal, /62\.73MB/);
assert.match(locales.es.statsTotal, /62\.73MB/);
